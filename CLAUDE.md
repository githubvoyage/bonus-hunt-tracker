...
src/
  ...
  components/
    ...
    EditableValue.jsx      pole edytowane w miejscu (przerywana ramka, commit na Enter/blur); opcjonalny `guard(onYes, onNo)` pyta o potwierdzenie na blur (przez ConfirmModal, nie window.confirm), tylko gdy wartość faktycznie się zmieniła, i cofa wpis przy odmowie
    ConfirmModal.jsx       własne okienko „na pewno?" zamiast window.confirm() — patrz sekcja niżej, dlaczego
...
- **Ochrona zakończonego hunta**: `App.jsx` ma `guardFinished(action, onConfirm, onCancel)` — jeśli `activeHunt.finished`, pyta przez `ConfirmModal` (nie `window.confirm()`) przed każdą zmianą (dodanie/usunięcie slota, bet, wygrana, nazwa/waluta/kasa na start, ekipa); jeśli nie jest zakończony, od razu woła `onConfirm()`. Dla przycisków guard opakowuje właściwą mutację w callbacku w samym handlerze w `App.jsx`. Dla pól `EditableValue` (nazwa, waluta, kasa na start, bet) guard odpala się na **blur, nie na focus** — i tylko gdy wartość faktycznie się zmieniła — i dostaje `(onYes, onNo)` zamiast zwracać bool.
  - **Dlaczego nie `window.confirm()`:** dwa realne bugi zgłoszone przez użytkownika. (1) `confirm()` wywołany z handlera `onFocus` wchodzi w nieskończoną pętlę — przeglądarka po zamknięciu dialogu oddaje focus z powrotem na to samo pole, co odpala focus (i confirm()) jeszcze raz. (2) w niektórych przeglądarkach (np. webview w aplikacjach typu Messenger/Instagram) `window.confirm()` jest zablokowany i zawsze cicho zwraca `false`, więc KAŻDA chroniona akcja wyglądała jakby była na stałe zablokowana. Własny `ConfirmModal.jsx` (zwykły komponent Reacta, stan w `App.jsx` jako `confirmState`) działa identycznie wszędzie i nie ma tych problemów. Nie zawracaj do `window.confirm()` przy poprawkach tego mechanizmu.
