...
src/
  ...
  components/
    ...
    EditableValue.jsx      pole edytowane w miejscu (przerywana ramka, commit na Enter/blur); opcjonalny `guard` pyta o potwierdzenie na blur, tylko gdy wartość faktycznie się zmieniła, i cofa wpis przy odmowie
...
- **Ochrona zakończonego hunta**: `App.jsx` ma `guardFinished(action)` — jeśli `activeHunt.finished`, pyta `confirm()` przed każdą zmianą (dodanie/usunięcie slota, bet, wygrana, nazwa/waluta/kasa na start, ekipa). Dla przycisków guard siedzi w samym handlerze w `App.jsx`. Dla pól `EditableValue` (nazwa, waluta, kasa na start, bet) guard odpala się na **blur, nie na focus** — i tylko gdy wartość faktycznie się zmieniła. **Nie przenoś tego z powrotem na `onFocus`**: `confirm()` wywołany z handlera focusu wchodzi w pętlę, bo przeglądarka po zamknięciu dialogu oddaje focus z powrotem na to samo pole, co odpala focus (i confirm()) jeszcze raz w nieskończoność — to był realny bug zgłoszony przez użytkownika.
